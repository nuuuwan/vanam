...
function parseSpeciesName(name) {
  ...
  for (const token of tokens) {
    if (inAuthority) {
      segments.push({ text: token, italic: false });
      continue;
    }
    ...
    if (CONNECTORS.has(token)) {
      ...
      segments.push({ text: normalized, italic: true });
    } else if (/^[a-z]/.test(token)) {
      // Epithet / infraspecific epithet — lowercase, italic
      segments.push({ text: token.toLowerCase(), italic: true });
    } else {
...
const SpeciesNameView = ({ species, variant = "body1", sx = {}, noWrap, href }) => {
  ...
  const nodes = segments.map((seg, i) => {
    ...
    return seg.italic ? (
      ...
      <span key={i} style={inAuthority || !seg.isConnector ? { opacity: 0.5 } : undefined}>
        {prefix}
        {seg.text}
      </span>
    );
